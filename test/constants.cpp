...
{
    EXPECT_EQ((VEC2F_ZERO).x, 0.0f);
    EXPECT_EQ((VEC2F_ZERO).y, 0.0f);
}

TEST(Constants, vec2f_one)
{
    EXPECT_EQ((VEC2F_ONE).x, 1.0f);
    EXPECT_EQ((VEC2F_ONE).y, 1.0f);
}

TEST(Constants, vec2f_up)
{
    EXPECT_EQ((VEC2F_UP).x,  0.0f);
    EXPECT_EQ((VEC2F_UP).y, -1.0f);
}

TEST(Constants, vec2f_left)
{
    EXPECT_EQ((VEC2F_LEFT).x, -1.0f);
    EXPECT_EQ((VEC2F_LEFT).y,  0.0f);
}

TEST(Constants, vec2f_down)
{
    EXPECT_EQ((VEC2F_DOWN).x, 0.0f);
    EXPECT_EQ((VEC2F_DOWN).y, 1.0f);
}

TEST(Constants, vec2f_right)
{
    EXPECT_EQ((VEC2F_RIGHT).x, 1.0f);
    EXPECT_EQ((VEC2F_RIGHT).y, 0.0f);
}
